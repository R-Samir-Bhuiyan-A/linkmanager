...
router.patch(
  "/:id",
  auth,
  requireRole(["Owner", "Admin", "Moderator", "Manage-only"]),
  async (req, res) => {
    try {
      const project = await Project.findById(req.params.id);
      if (!project)
      ...
      if (req.user.role === "Manage-only") {
        if (!project.assignedUsers.includes(req.user.id)) {
          return res
            .status(403)
            .json({
              message: "Access denied: Not assigned to manage this project",
            });
        }
      }
      ...
      if (req.body.maintenanceMode !== undefined)
        project.maintenanceMode = req.body.maintenanceMode;
      if (req.body.customHeaders !== undefined)
        project.customHeaders = req.body.customHeaders;
      if (req.body.latestVersion !== undefined)
        project.latestVersion = req.body.latestVersion;
      if (req.body.minVersion !== undefined)
        project.minVersion = req.body.minVersion;
      if (req.body.updateUrl !== undefined)
        project.updateUrl = req.body.updateUrl;

      // Owner/Admin only fields
      if (["Owner", "Admin"].includes(req.user.role)) {
        if (req.body.assignedUsers !== undefined)
          project.assignedUsers = req.body.assignedUsers;
      }

      if (req.body.clientAuth) {
        if (!project.clientAuth) project.clientAuth = {};
        if (req.body.clientAuth.enabled !== undefined)
          project.clientAuth.enabled = req.body.clientAuth.enabled;
        if (req.body.clientAuth.publicFields !== undefined)
          project.clientAuth.publicFields = req.body.clientAuth.publicFields;
      }
...
router.post(
  "/:id/secret/reveal",
  auth,
  requireRole(["Owner", "Admin", "Moderator"]),
  async (req, res) => {
    try {
      ...
      if (req.user.role === "Manage-only") {
        if (!project.assignedUsers.includes(req.user.id)) {
          return res
            ...
            .json({
              message:
                "Access denied: You are not assigned to manage this project",
            });
        }
      }
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      project.apiKeys = project.apiKeys.filter(
        (k) => k._id.toString() !== req.params.keyId,
      );
      await project.save();
